<html>
	<head>
  	<title> 読み取り履歴 </title> <meta charset="UTF-8">
    <link href="css/rireki.css" rel="stylesheet" type="text/css" />
    <script type="text/javascript" src="js/jquery-3.1.1.min.js"></script>
    <script type="text/javascript" src="js/rireki.js"></script>
    <script type="text/javascript">

    $(function() {
      $('.Layout').tile(3);
    });
    </script>
	</head>

  <body>
		<br>
    <h1> 読み取り履歴 </h1>
    <?php
			/////////////////////////////////////
      // リロード時の二重送信防止 (データ処理) //
      /////////////////////////////////////
			if ($_SERVER['REQUEST_METHOD'] === 'POST') {
				///////////////////////
				// お気に入り登録の処理 //
				//////////////////////

				//rireki.phpのお気に入り追加ボタンから送られてきたPOST型の変数
				if (isset($_POST["num"]) && isset($_POST["name"]) && isset($_POST["img"])) {
					$num = $_POST["num"];
					$name = $_POST["name"];
					$img = $_POST["img"];

					$count = 0;

					// JANコードからレコード取得
					$pdo = new PDO("sqlite:CMP.sqlite");
					$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
					$st = $pdo->query("select * from favorite where num =" . $num);
					foreach($st as $row){
					 $count++;
					}

					// 件数比較
					if ($count == 0) {
						// データの追加
						$st = $pdo->prepare("insert into favorite(num, name, img) values(?, ?, ?)");
						$st->execute(array($num, $name, $img));
					}
					else {
						// すでにリストに入ってる。
						// do nothing...
					}
				}
				/////////////////
				// 履歴削除の処理//
				////////////////

				//rireki.phpの削除ボタンから送られてきたPOST型の変数
				else if (isset($_POST["num"])) {
					$num = $_POST["num"];

					$pdo = new PDO("sqlite:CMP.sqlite");
					$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
					$st = $pdo->query("delete from rireki where num =" . $num);
				}
				header('Location:rireki.php', true, 303);
			}
			/////////////////////////////////////
      // リロード時の二重送信防止 (結果の表示) //
      /////////////////////////////////////
			else {
				// データベース読み込み
				$db = new PDO("sqlite:CMP.sqlite");
				$result=$db->query("SELECT * FROM rireki");
				// 履歴一覧表示
	      echo '<div id="wrap">';
	      echo '<div class="tiles_2">';

	      for($i = 1; $row=$result->fetch(); ++$i ){
	        echo '<div class="Layout">';
	        echo '<a href="https://www.amazon.co.jp/s/url=search-alias%3Daps&field-keywords=' . $row["num"]
	             . '" style="text-decoration: none;" class="btn-action"> ' . '<img src=' . $row["img"] . '/>';
	        echo '<h3>' . $row["name"] . '</h3>';
	        echo '</a>';
					//ほしいものリスト追加ボタン
					echo '<form action="rireki.php" method="POST">';
					echo '<div class="button"><button type="submit">ほしいものリストに追加</button></div>';
	        echo '<input type="hidden" name="num" value="' . $row["num"] . '">';
					echo '<input type="hidden" name="name" value="' . $row["name"] . '">';
					echo '<input type="hidden" name="img" value="' . $row["img"] . '">';
					echo '</form>';
	        // 削除ボタン
					echo '<br>';
	        echo '<form action="rireki.php" method="POST">';
					echo '<div class="button"><button type="submit">削除する</button></div>';
	        echo '<input type="hidden" name="num" value="' . $row["num"] . '">';
					echo '</form>';
	        echo '</div>';
	      }

	      echo '</div>';
	      echo '</div>';
				echo '<h2><a href="favorite.php" style="text-decoration: none; color: white;">
							ほしいものリストを見る</a></h2>';
			}
    ?>
  </body>
</html>
